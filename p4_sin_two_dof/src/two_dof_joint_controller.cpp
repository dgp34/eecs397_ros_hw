...
{ 
  ROS_INFO("Received pos_cmd is: %f",pos_cmd_msg.data); 
  g_pos_cmd = pos_cmd_msg.data;
} 

void posCmd2CB(const std_msgs::Float64& pos_cmd_msg) 
{ 
  ROS_INFO("Received pos_cmd2 is: %f",pos_cmd_msg.data); 
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "two_dof_joint_controller");
    ros::NodeHandle nh;
    ...
    ros::Subscriber pos_cmd_subscriber2 = nh.subscribe("pos_cmd2",1,posCmd2CB); 
    ...
    double q2, q2dot, q2_err, trq_cmd2;
    ...
    joint_state_publisher.publish(joint_state_msg);
        ...
        trq_publisher2.publish(trq_msg);
        // send torque command to Gazebo
