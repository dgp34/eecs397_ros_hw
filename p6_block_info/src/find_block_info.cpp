...
vector <double> g_minmax(6); //will hold min & max of each dimension of block
...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan_in) {
    ...
    //run through each point in the frame to find
    // min and max coordinates of each dimension of block
    for (int i = 0; i < npts3d; i++) {
        ...
        if (!(vec[2]< 0.1)) {
          ...
          //combined display and calculation step
          //dimensions are calculated by subtracting
          // the min coord componenet from the max coord componenet
          // while centroid is found by adding min coord to
          //  half of the corresponding block dimension for x and y
          //  and simply dividing the height of the block by two for z
          ROS_INFO("Block dimensions are, in x-y-z:  %f by %f by %f. Centroid in world frame is located at (x,y,z) = (%f, %f, %f). \n", (g_minmax[1] - g_minmax[0]), (g_minmax[3] - g_minmax[2]), (g_minmax[5] - g_minmax[4]), g_minmax[0] + (g_minmax[1] - g_minmax[0])/2.0, g_minmax[2] + (g_minmax[3] - g_minmax[2])/2.0, (g_minmax[5] - g_minmax[4])/2.0);
        }
    }
}
