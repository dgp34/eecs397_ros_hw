//MANIPULATOR OPTIONS FOR STICKY-FINGERS GRIPPER:
//for each combination of gripper/object, need a function like this:

void virtual_vacuum_grasp_TOY_BLOCK_ID(int query_code, int grasp_option,
    ...
    object_pose_wrt_gripper.position.z = 0.0175; //0.035/2; touch top face of block
    ...
    switch (query_code) {
        ...
        case object_manipulation_properties::objectManipulationQueryRequest::GET_GRASP_POSE_TRANSFORMS:
            if (grasp_option == object_manipulation_properties::objectManipulationQueryRequest::GRASP_FROM_ABOVE) {
                ...
                ROS_WARN("this grasp option not specified for virtual vacuum/sticky-fingers gripper and object TOY_BLOCK_ID");
        ...
        default:
            ROS_WARN("grasp poses for virtual vacuum/sticky-fingers gripper unknown for object code TOY_BLOCK_ID");
            response.valid_reply = false;
            break;
    }
...
/* Begin copy-pasta from above, editing magic values and comments as appropriate */
void virtual_vacuum_grasp_ARIAC_PULLEY(int query_code, int grasp_option,
        object_manipulation_properties::objectManipulationQueryResponse&response) {
    ...
    Eigen::Vector3d x_axis, y_axis, z_axis;
    XformUtils xformUtils;
    ROS_INFO("query, vacuum gripper, ariac pulley; query code %d, grasp option %d", query_code, grasp_option);
    //here are two grasp poses using baxter gripper to grasp ariac pulley:
    object_pose_wrt_gripper.position.x = 0.0; //easy--align gripper origin w/ object origin
    object_pose_wrt_gripper.position.y = 0.0;
    object_pose_wrt_gripper.position.z = 0.0175; //0.035/2; touch top face of pulley (z=0.035145)
    object_pose_wrt_gripper.orientation.x = 1.0; //x-axis parallel, z-axis anti-parallel:
    object_pose_wrt_gripper.orientation.y = 0.0; //grasp from above
    object_pose_wrt_gripper.orientation.z = 0.0;
    object_pose_wrt_gripper.orientation.w = 0.0;
    ...
    switch (query_code) {
        ...
        case object_manipulation_properties::objectManipulationQueryRequest::GET_GRASP_POSE_TRANSFORMS:
            if (grasp_option == object_manipulation_properties::objectManipulationQueryRequest::GRASP_FROM_ABOVE) {
                ...
                response.valid_reply = true;
                break;
            } else { //this is where to consider alternative grasp strategies for ARIAC_PULLEY using RETHINK gripper
                ROS_WARN("this grasp option not specified for virtual vacuum/sticky-fingers gripper and object ARIAC_PULLEY");
                response.valid_reply = false;
                break;
            }
        ...
        default:
            ROS_WARN("grasp poses for virtual vacuum/sticky-fingers gripper unknown for object code ARIAC_PULLEY");
            response.valid_reply = false;
            break;
    }
...
void sticky_fingers_grasp_query(int object_id, int query_code, int grasp_option,
        object_manipulation_properties::objectManipulationQueryResponse&response) {
    switch (object_id) {
        case ObjectIdCodes::TOY_BLOCK_ID:
            virtual_vacuum_grasp_TOY_BLOCK_ID(query_code, grasp_option, response);
            break;
        case ObjectIdCodes::ARIAC_PULLEY:
            virtual_vacuum_grasp_ARIAC_PULLEY(query_code, grasp_option, response);
            break;
     /* case OBJECT_ID: add more cases here, for each object to be grasped by rethink gripper...need a separate function to fill response for each object/gripper combo */
